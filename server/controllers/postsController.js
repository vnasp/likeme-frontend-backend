  ...
  try {
    ...
    if (code == "23502") {
      ...
      res
        .status(400)
        .send(
          "Bad Request: el servidor no puede procesar la solicitud debido a un error del cliente"
        );
    } else {
...
const addPosts = async (req, res) => {
  try {
    const { title, image, about } = req.body;
    const newPost = await addPost(title, image, about);
